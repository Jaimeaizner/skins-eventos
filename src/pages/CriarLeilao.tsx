...
export default function CriarLeilao() {
  ...
  const validateForm = () => {
    ...
    if (!formData.startingBid || parseFloat(formData.startingBid) <= 0) {
      newErrors.startingBid = 'Lance inicial deve ser maior que 0';
    }

    if (!formData.minBidIncrement || parseFloat(formData.minBidIncrement) <= 0) {
      newErrors.minBidIncrement = 'Incremento mínimo deve ser maior que 0';
    }
    ...
    // Validar se o lance inicial é menor que o preço Steam
    if (formData.steamPrice && formData.startingBid) {
      if (parseFloat(formData.startingBid) >= parseFloat(formData.steamPrice)) {
        newErrors.startingBid = 'Lance inicial deve ser menor que o preço Steam';
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Simular criação do leilão
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Aqui você faria a chamada para a API
      console.log('Leilão criado:', formData);
      
      // Redirecionar para a página de leilões
      navigate('/leiloes');
    } catch (error) {
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <img src="/image/Leilão.png" alt="Criar Leilão" className="max-h-16 object-contain drop-shadow-lg" />
              <div>
                <h1 className="text-3xl font-bold text-white">Criar Novo Leilão</h1>
                <p className="text-gray-300">Preencha os dados do seu leilão</p>
              </div>
            </div>
            <button
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-black bg-opacity-40 backdrop-blur-md rounded-2xl border border-white border-opacity-20 p-6">
            <h2 className="text-2xl font-bold text-white mb-6">Dados do Leilão</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-white font-semibold mb-2">Lance Inicial (R$)</label>
                  <input
                    type="number"
                    name="startingBid"
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-white bg-opacity-10 border rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-300 ${
                      errors.startingBid ? 'border-red-500' : 'border-white border-opacity-20'
                    }`}
                    placeholder="0.00"
                    step="0.01"
                  />
                  {errors.startingBid && <p className="text-red-400 text-sm mt-1">{errors.startingBid}</p>}
                </div>
              </div>

              {/* Incremento mínimo e Data */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-white font-semibold mb-2">Incremento Mínimo (R$)</label>
                  <input
                    type="number"
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-white bg-opacity-10 border rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-300 ${
                      errors.minBidIncrement ? 'border-red-500' : 'border-white border-opacity-20'
                    }`}
                    placeholder="1.00"
                    step="0.01"
                    min="0.01"
                  />
                  {errors.minBidIncrement && <p className="text-red-400 text-sm mt-1">{errors.minBidIncrement}</p>}
                </div>
                <div>
              ...
              <div className="p-4 relative">
                ...
                <div className="mb-3 space-y-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-green-400 font-bold">R$ {formData.startingBid || '0,00'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-300">Incremento mínimo:</span>
                    <span className="text-blue-400 font-bold">R$ {formData.minBidIncrement || '0,00'}</span>
                  </div>
                </div>
                <div className="flex justify-between text-sm text-gray-300 mb-2">
                  <span className="font-semibold">Data de término</span>
                  <span className="font-bold text-white">{formData.endDate && formData.endTime ? `${formData.endDate} ${formData.endTime}` : '---'}</span>
                </div>
                <div className="mb-4">
